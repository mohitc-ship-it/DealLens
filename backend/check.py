...
#     images = []
#     for chunk in chunks:
#         # Check if chunk has 'image' attribute
#         if hasattr(chunk, "image") and chunk.image is not None:
#             img_bytes = chunk.image
...
#     images = get_images_from_chunks(chunks)
#     print("len ",len(images))

#     if show_images and images:
#         print(f"Found {len(images)} images, displaying...")
#         for img in images:
#             img.show()  # opens in default image viewer

#     return chunks, tables, texts, images
# # --- Example usage ---
# if __name__ == "__main__":
#     lissst = ['220943.pdf']
#     # file_path = "../Kansas compliance codes/Ordinance No. 130734 (Committee Substitute).pdf"
#     for f in lissst:
#         file_path = "../Kansas compliance codes/"+f
#         chunks, tables, texts, images = chunking(file_path)
#         print(f"Chunks: {len(chunks)}, Tables: {len(tables)}, Texts: {len(texts)}, Images: {len(images)}")
from unstructured.partition.pdf import partition_pdf
...
MIN_WIDTH = 700   # ignore images smaller than this width
MIN_HEIGHT = 100  # ignore images smaller than this height
...
# --- Example usage ---
if __name__ == "__main__":
    file_path = "../Kansas compliance codes/Ordinance No. 170042.pdf"
